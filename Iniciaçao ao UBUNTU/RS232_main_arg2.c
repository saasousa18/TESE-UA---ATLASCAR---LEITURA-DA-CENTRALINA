...
int escrever_serial(char *nome_porta, unsigned char data){
...
{  
	...
	if(argc==2){ // é 2 dois porque o nome do ficheiro ocupa logo a primeira 		posiçao
		if(num==0){
					...
					escrever_serial( "/dev/ttyUSB0", data);//abre a porta USB0 e escreve ola
					//if( port0 != -1);				
				  }
		if(num==1){
